...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    # Create a new dictionary of positions/values
    allPossible = util.Counter()
    # Update all positions with probabilities
    for p in self.legalPositions:
      # Get the Manhattan distance between current position and Pacman
      trueDistance = util.manhattanDistance(p, pacmanPosition)
      # If there is any possibility of a ghost at current position
      if emissionModel[trueDistance] > 0.0:
        # Value at position is weighted by the current probability
        allPossible[p] = emissionModel[trueDistance] * self.beliefs[p]
    # Normalize values between 0.0 and 1.0
    allPossible.normalize()
    # Update position/value dictionary
  ...
  def elapseTime(self, gameState):
    ...
    """
    # New dictionary for positions/probabilities
    allPosDist = util.Counter()
    # Update values for every position where there is some possibility of a ghost
    for p in self.legalPositions:
      if self.beliefs[p] > 0.0:
        # A small dictionary of posisions that a ghost at the current posision could end up
        # Weighted by probability
        newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
        # Wights newPosDist by the probability that the ghost was at the current position
        for q in newPosDist:
          allPosDist[q] += newPosDist[q] * self.beliefs[p]
    # Normalized values between 0.0 and 1.0
    allPosDist.normalize()
    # Updates position/probability dictionary
    self.beliefs = allPosDist
